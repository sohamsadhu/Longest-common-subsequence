...
void Hirschberg ::read_write()
{
	...
	while (!myfile.eof())
	{
		...
		cout << "case " << case_num << " for string length " << len1 << " "
				 << " lcs length is " << s4.length() << " and lcs is " << s4
				 << " time taken " << diff(ts, te).tv_sec << ":"
	...
	while (!myfile.eof())
	{
		...
		cout << "case " << case_num << " for string length " << len1
				 << " lcs length is " << s4.length() << " and lcs is " << s4
				 << " time taken " << diff(ts, te).tv_sec << ":" << diff(ts, te).tv_nsec << endl;
		case_num++;
	}
	myfile.close();
}
