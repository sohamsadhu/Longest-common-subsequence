...
void LCS :: dyn_prog( string A, string B )
{
	...
	string lcs = "" ;
	// Traverse the table and put the characters of the LCS that match into the empty string lcs that will be retruned or printed.
    ...
    cout << " the length of lcs is " << lcs.length() << " for string length " << A.length()  <<" and the lcs is: " << " " ;
...
void LCS :: read_write()
{
    string A , B , line ; // A, B for the strings that have LCS and line for getting the line from file that has both the strings
    ifstream myfile ;
    int case_num = 1 ;
