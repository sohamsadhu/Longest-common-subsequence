...
// write the LCS length. The other will calculate the just the LCS via naive recursive algorithm.
...
int LCS ::lcs_len(string A, string B)
{
  ...
  if (A[A.length() - 1] == B[B.length() - 1]) return lcs_len(c, d) + 1;
  else                                        return max(lcs_len(c, B), lcs_len(A, d));
}
...
void LCS ::lcs_read_write()
{
  ...
  if (myfile.is_open())
  {
    while (myfile.good())
    {
      ...
      cout << " case " << case_num << " lcs length is: " << lcs_len(A, B) 
        << " for strings of length " << A.length();
